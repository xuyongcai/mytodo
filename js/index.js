...
function create_html() {
	...
	for(var i = 0; i < task_list.length; i++) {
		if(task_list[i].completed) { //选中了
			...
			clock_time($item); //闹钟提示
		}
	}
	for(var j = complete_items.length - 1; j >= 0; j--) {
		//      console.log(complete_items[j])

		if(complete_items[j]) { //排除undefined，这个很重要
			$item = bind_html(complete_items[j], j);
			$item.addClass("completed"); //给li对象添加类
...
function bind_html(data, index) {
	...
	var str = '<li data-index = "' + index + '">' +
		'<input type="checkbox" ' + (data.completed ? "checked" : "") + ' class="ckd" />' +
...
function bind_detail() {
	$(".detail").click(function() {
		...
		var index = $oli.data("index");
		var obj = task_list[index];

		create_detail_html(obj, index);
...
function create_detail_html(obj, index) {
	var $task_list = $(".task-list");
	$task_list.find("li").each(function(index, ele) {
		if($(ele).find("span").eq(0).text() == obj.title) { //得到特定li对象

			$(".task-list li div").remove(".detail-box"); //关闭原来弹窗
			var src = bind_detail_html(obj, index); //弹窗
			$(ele).append(src);
			
			jQuery('#datetimepicker').datetimepicker(); //引用时间插件
...
//创建DetailHtml
function bind_detail_html(obj, index) {
	var src = '<div class="detail-box">' +
		...
		'<span>提醒时间</span>' +
		'<input type="text" id="datetimepicker" class="detail-time" value="' + (obj.time || "") + '" />' +
...
//2.过滤
//3.end_time= 结束时间   task_list[i].time
//得到毫秒
//到时间了， 播放音乐 提醒    显示黄色条
//关闭音乐
var timer = null;
...
//	console.log($(obj)[0])
	...
	$(obj)[0].timer = setInterval(function() {
		...
		//过滤
		for(var i = 0; i < task_list.length; i++) {
			if(!task_list[i].completed || !task_list[i].time || task_list[i].off) continue;
//			console.log(5)
			...
			if(end_time - start_time <= 0) {
				...
				clearInterval($(obj)[0].timer);
				play_music(true);
				show_alert(task_list[i],$(obj)[0],i);
...
//任务提示
//show_alert(task_list[0], 0);

function show_alert(obj,obj1,index) {
//	console.log(obj1)
	$(".msg").show();
	$(".msg").find("span").click(function() {
		clearInterval(obj1.timer);
		//清除选择的文字
	    window.getSelection ? window.getSelection().removeAllRanges() : document.selection.empty();     
		up_data({off: true},index)
		$(".msg").hide();
		play_music(false);
		console.log($(this).index())
	    $(this).index() == 0 ? create_detail_html(obj,index) : index;
	})

}
//音乐控制
function play_music(off) {
	var music = document.getElementById("music");
	off ? music.play() : music.pause();
}
